...
#define _Steps  10. //disk texture layers
...
{
	// angle.x is yaw (around Y), angle.y is pitch (around X)
...
void main()
{
    ...
    for( int i=0; i<AA; i++ )
    {
        // New camera setup with drag (iRotation) and wheel (iZoom) controls.
        vec3 ray = normalize(vec3((fragCoord - iResolution.xy * 0.5 + vec2(i,j) / float(AA)) / iResolution.x, 1.0)); 
        vec3 pos = vec3(0.0, 0.05, -iZoom);
        
        // iRotation.y is yaw (horizontal drag), iRotation.x is pitch (vertical drag).
        // The shader's Rotate function uses angle.x for yaw and angle.y for pitch.
        vec2 angle = vec2(iRotation.y + iTime * 0.05, iRotation.x);
        ...
        for(int disks = 0; disks< 15; disks++) //steps
        {
